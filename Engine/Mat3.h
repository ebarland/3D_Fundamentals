...
public:
	...
	{
		const T cosTheat = cos(theta);
		const T sinTheta = sin(theta);

		_Mat3 r =
		{
			(T)1.0, (T)0.0,	   (T)0.0,
			(T)0.0, cosTheat,  sinTheta,
			(T)0.0, -sinTheta, cosTheat
		};
		return r;
	}
	static _Mat3 RotationY(T theta)
	{
		...
		_Mat3 r =
		{
			cosTheat, (T)0.0, -sinTheta,
			(T)0.0,   T(1.0), T(0.0),
			sinTheta, T(0.0), cosTheat
		};
		return r;
	}
	static _Mat3 RotationZ(T theta)
	{
		...
		_Mat3 r =
		{
			cosTheat,  sinTheta, (T)0.0,
			-sinTheta, cosTheat, (T)0.0,
			(T)0.0,	  (T)0.0,    (T)1.0
		};
		return r;
	}
...
_Vec3<T> operator*(const _Vec3<T> &lhs, const _Mat3<T> &rhs)
{
	return
	{
		lhs.x * rhs.elements[0][0] + lhs.y * rhs.elements[0][1] + lhs.z * rhs.elements[0][2],
		lhs.x * rhs.elements[1][0] + lhs.y * rhs.elements[1][1] + lhs.z * rhs.elements[1][2],
		lhs.x * rhs.elements[2][0] + lhs.y * rhs.elements[2][1] + lhs.z * rhs.elements[2][2]
	};
}
